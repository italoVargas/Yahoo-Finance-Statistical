...
def YHF(T,Ai,Af):
	...
	for i in dias:	
		try:
			...
			for m in meses:
				...
				if m==1:
					M1.append(j.Open.mean())
				if m==2:
					M2.append(j.Open.mean())
	
				if m==3:
					M3.append(j.Open.mean())
				if m==4:
					M4.append(j.Open.mean())
	
				if m==5:
					M5.append(j.Open.mean())
				if m==6:
					M6.append(j.Open.mean())
	
				if m==7:
					M7.append(j.Open.mean())
				if m==8:
					M8.append(j.Open.mean())
		
				if m==9:
					M9.append(j.Open.mean())
				if m==10:
					M10.append(j.Open.mean())
	
				if m==11:
					M11.append(j.Open.mean())
				if m==12:
					M12.append(j.Open.mean())
	...
	plt.savefig(str( T )+' Desde: '+str(Ai)+' Hasta: '+str(Af)+ 'Open' +'.png')
