...
def YHF(T,Ai,Af):
	...
	for i in dias:	
		try:
			...
			for m in meses:
				...
				if m==1:
					M1.append(j.Low.mean())
				if m==2:
					M2.append(j.Low.mean())
	
				if m==3:
					M3.append(j.Low.mean())
				if m==4:
					M4.append(j.Low.mean())
	
				if m==5:
					M5.append(j.Low.mean())
				if m==6:
					M6.append(j.Low.mean())
	
				if m==7:
					M7.append(j.Low.mean())
				if m==8:
					M8.append(j.Low.mean())
		
				if m==9:
					M9.append(j.Low.mean())
				if m==10:
					M10.append(j.Low.mean())
	
				if m==11:
					M11.append(j.Low.mean())
				if m==12:
					M12.append(j.Low.mean())
	...
	plt.savefig(str( T )+' Desde: '+str(Ai)+' Hasta: '+str(Af)+ ' Low' +'.png')
...
while True:
	YHF('BTC-USD','2015','2021')
	YHF('GENIUS21.MX','2015','2021')
	YHF('IVVPESOISHRS.MX','2015','2021')
	YHF('SPLG','2015','2021')
	YHF('^GSPC','2015','2021')
	YHF('VT','2015','2021')
	YHF('WALMEX.MX','2015','2021')
	YHF('SPY','2015','2021')
	YHF('VNQ','2015','2021')
	YHF('VOO','2015','2021')
	YHF('GMEXICOB.MX','2015','2021')
	YHF('BABA','2015','2021')
